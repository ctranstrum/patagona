...
(kicad_pcb
  ...
  (general
    (thickness ${params.custom.thickness || '1.6'})
    (legacy_teardrops no)
  )
  (paper "${params.custom.paper || 'A3'}")
  (title_block
    (title "${params.custom.name || params.name}")
    (date "${date_text}")
    (rev "${params.custom.version || params.version}")
    (company "${params.custom.company || params.author}")
  )
